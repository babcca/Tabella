...
namespace {/**
 * Nette Framework (version 2.0-dev released on 2011-05-12, http://nette.org)
 *
...
new
ApplicationException('Too many loops detected in application life cycle.');}if(!$request){$this->onStartup($this);$session=$this->context->session;if(!$session->isStarted()&&$session->exists()){$session->start();}$router=$this->getRouter();Diagnostics\RoutingPanel::initialize($this,$httpRequest);$request=$router->match($httpRequest);if(!$request
instanceof
...
run(Application\Request$request){try{$this->request=$request;$this->payload=(object)NULL;$this->setParent($this->getParent(),$request->getPresenterName());$this->initGlobalParams();$this->startup();if(!$this->startupCheck){$class=$this->getReflection()->getMethod('startup')->getDeclaringClass()->getName();throw
new
...
Configurator
extends
Object{public$defaultConfigFile='%appDir%/config.neon';public$defaultServices=array('application'=>array(__CLASS__,'createApplication'),'presenterFactory'=>array(__CLASS__,'createPresenterFactory'),'httpContext'=>array(__CLASS__,'createHttpContext'),'httpRequest'=>array(__CLASS__,'createHttpRequest'),'httpResponse'=>'Nette\Http\Response','user'=>array(__CLASS__,'createHttpUser'),'cacheStorage'=>array(__CLASS__,'createCacheStorage'),'cacheJournal'=>array(__CLASS__,'createCacheJournal'),'mailer'=>array(__CLASS__,'createMailer'),'session'=>array(__CLASS__,'createHttpSession'),'robotLoader'=>array(__CLASS__,'createRobotLoader'),'templateCacheStorage'=>array(__CLASS__,'createTemplateCacheStorage'));function
...
NULL;}}function
loadConfig(DI\IContainer$container,$file){$name=Environment::getName();if($file
instanceof
ArrayHash){$config=$file;$file=NULL;}else{if($file===NULL){$file=$this->defaultConfigFile;}$file=Environment::expand($file);if(!is_file($file)){$file=preg_replace('#\.neon$#','.ini',$file);}$config=Nette\Config\Config::fromFile($file,$name);}$container->config=$config;if(isset($config->variable)&&$config->variable
instanceof\Traversable){foreach($config->variable
as$key=>$value){Environment::setVariable($key,$value);}}$iterator=new\RecursiveIteratorIterator($config);foreach($iterator
as$key=>$value){$tmp=$iterator->getDepth()?$iterator->getSubIterator($iterator->getDepth()-1)->current():$config;$tmp[$key]=Environment::expand($value);}$runServices=array();if(isset($config->service)&&$config->service
instanceof\Traversable){foreach($config->service
as$key=>$value){$key=strtr($key,'-','\\');if(preg_match('#^Nette\\\\.*\\\\I?([a-zA-Z]+)$#',$key,$m)){$m[1][0]=strtolower($m[1][0]);trigger_error(basename($file).": service name '$key' has been renamed to '$m[1]'",E_USER_WARNING);$key=$m[1];}if(is_string($value)){$container->removeService($key);$container->addService($key,$value);}else{if(!empty($value->factory)||isset($this->defaultServices[$key])){$factory=empty($value->factory)?$this->defaultServices[$key]:$value->factory;if(!empty($value->option)){$factory=function()use($container,$factory,$value){return
call_user_func($factory,$container,(array)$value->option);};}$container->removeService($key);$container->addService($key,$factory);}else{throw
new
Nette\InvalidStateException("Factory method is not specified for service $key.");}if(!empty($value->run)){$runServices[]=$key;}}}}if(!isset($config->php)&&isset($config->set)){$config->php=$config->set;unset($config->set);}if(isset($config->php)&&$config->php
instanceof\Traversable){if(PATH_SEPARATOR!==';'&&isset($config->php->include_path)){$config->php->include_path=str_replace(';',PATH_SEPARATOR,$config->php->include_path);}foreach(clone$config->php
as$key=>$value){if($value
instanceof\Traversable){unset($config->php->$key);foreach($value
...
new
Nette\NotSupportedException('Required function ini_set() is disabled.');}}}}}if(isset($config->const)&&$config->const
instanceof\Traversable){foreach($config->const
as$key=>$value){define($key,$value);}}if(isset($config->mode)&&isset($config->mode)){foreach($config->mode
as$mode=>$state){$container->setParam($mode.'Mode',(bool)$state);}}foreach($runServices
as$name){$container->getService($name);}return$config;}function
createContainer(){$container=new
DI\Container;foreach($this->defaultServices
as$name=>$service){$container->addService($name,$service);}defined('APP_DIR')&&$container->setParam('appDir',APP_DIR);defined('LIBS_DIR')&&$container->setParam('libsDir',LIBS_DIR);defined('TEMP_DIR')&&$container->setParam('tempDir',TEMP_DIR);$container->setParam('productionMode',$this->detect('production'));return$container;}static
function
createApplication(DI\IContainer$container,array$options=NULL){$context=new
DI\Container;$context->addService('httpRequest',$container->httpRequest);$context->addService('httpResponse',$container->httpResponse);$context->addService('session',$container->session);$context->addService('presenterFactory',$container->presenterFactory);$context->addService('router','Nette\Application\Routers\RouteList');Nette\Application\UI\Presenter::$invalidLinkMode=$container->getParam('productionMode')?Nette\Application\UI\Presenter::INVALID_LINK_SILENT:Nette\Application\UI\Presenter::INVALID_LINK_WARNING;$class=isset($options['class'])?$options['class']:'Nette\Application\Application';$application=new$class($context);$application->catchExceptions=$container->getParam('productionMode');return$application;}static
function
createPresenterFactory(DI\IContainer$container){return
new
Nette\Application\PresenterFactory($container->getParam('appDir'),$container);}static
function
createHttpRequest(){$factory=new
Nette\Http\RequestFactory;$factory->setEncoding('UTF-8');return$factory->createHttpRequest();}static
function
createHttpContext(DI\IContainer$container){return
new
Nette\Http\Context($container->httpRequest,$container->httpResponse);}static
function
createHttpSession(DI\IContainer$container){return
new
Nette\Http\Session($container->httpRequest,$container->httpResponse);}static
function
createHttpUser(DI\IContainer$container){$context=new
...
function
createCacheStorage(DI\IContainer$container){$dir=$container->expand('%tempDir%/cache');umask(0000);@mkdir($dir,0777);return
new
Nette\Caching\Storages\FileStorage($dir,$container->cacheJournal);}static
function
createTemplateCacheStorage(DI\IContainer$container){$dir=$container->expand('%tempDir%/cache');umask(0000);@mkdir($dir,0777);return
new
Nette\Caching\Storages\PhpFileStorage($dir);}static
function
createCacheJournal(DI\IContainer$container){return
new
Nette\Caching\Storages\FileJournal($container->getParam('tempDir'));}static
function
createMailer(DI\IContainer$container,array$options=NULL){if(empty($options['smtp'])){return
new
Nette\Mail\SendmailMailer;}else{return
new
Nette\Mail\SmtpMailer($options);}}static
function
createRobotLoader(DI\IContainer$container,array$options=NULL){$loader=new
Nette\Loaders\RobotLoader;$loader->autoRebuild=isset($options['autoRebuild'])?$options['autoRebuild']:!$container->getParam('productionMode');$loader->setCacheStorage($container->cacheStorage);if(isset($options['directory'])){$loader->addDirectory($options['directory']);}else{foreach(array('appDir','libsDir')as$var){if($dir=$container->getParam($var,NULL)){$loader->addDirectory($dir);}}}$loader->register();return$loader;}}class
DateTime
...
function
setMode($mode,$value=TRUE){self::getContext()->setParam($mode.'Mode',(bool)$value);}static
function
getMode($mode){if(isset(self::getContext()->params[$mode.'Mode'])){return
self::getContext()->params[$mode.'Mode'];}else{return
self::getContext()->params[$mode.'Mode']=self::getConfigurator()->detect($mode);}}static
function
isConsole(){return
...
function
setVariable($name,$value,$expand=TRUE){$expand=$expand&&is_string($value)&&strpos($value,'%')!==FALSE;if(!$expand){self::getContext()->setParam($name,$value);}self::$vars[$name]=array($value,$expand);}static
function
...
function
getContext(){if(self::$context===NULL){self::$context=self::getConfigurator()->createContainer();}return
self::$context;}static
function
...
self::getConfigurator()->loadConfig(self::getContext(),$file);}static
function
getConfig($key=NULL,$default=NULL){$config=self::getContext()->config;if(func_num_args()){return
isset($config[$key])?$config[$key]:$default;}else{return$config;}}}final
class
Framework{const
NAME='Nette Framework',VERSION='2.0-dev',REVISION='c27225a released on 2011-05-12';public
...
new
Nette\InvalidStateException("There is not section [$section] in file '$file'.");}$data=$data[$section];}return
Nette\ArrayHash::from($data,TRUE);}static
function
...
Connection
extends
PDO{private$driver;private$preprocessor;public$databaseReflection;public$cache;public$substitutions=array();public$onQuery;public$supportsMeta;function
__construct($dsn,$username=NULL,$password=NULL,array$options=NULL){parent::__construct($dsn,$username,$password,$options);$this->setAttribute(PDO::ATTR_STATEMENT_CLASS,array('Nette\Database\Statement',array($this)));$this->supportsMeta=(bool)@$this->query('SELECT 1')->getColumnMeta(0);$this->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);$class='Nette\Database\Drivers\\'.$this->getAttribute(PDO::ATTR_DRIVER_NAME).'Driver';if(class_exists($class)){$this->driver=new$class($this,(array)$options);}$this->preprocessor=new
...
normalizeRow($row){if($this->types===NULL){$this->types=array();if($this->connection->supportsMeta){foreach($row
...
implements
IContainer{private$params=array();private$registry=array();private$factories=array();private$types=array();private$creating;function
addService($name,$service,$typeHint=NULL){$this->updating();if(!is_string($name)||$name===''){throw
new
...
new
Nette\InvalidStateException("Service '$name' has already been registered.");}if(is_string($service)&&strpos($service,':')===FALSE){$typeHint=$typeHint?:$service;$service=new
...
instanceof
Nette\Callback){$this->registry[$name]=$service;$this->types[$name]=$typeHint;return$this;}else{$factory=$service;}$this->factories[$name]=array(callback($factory));$this->types[$name]=$typeHint;$this->registry[$name]=&$this->factories[$name][1];return$service;}function
removeService($name){$this->updating();unset($this->registry[$name],$this->factories[$name]);}function
getService($name){if(isset($this->registry[$name])){return$this->registry[$name];}if(isset($this->creating[$name])){throw
new
...
new
Nette\UnexpectedValueException("Unable to create service '$name', value returned by factory '$factory' is not object.");}elseif(isset($this->types[$name])&&!$service
instanceof$this->types[$name]){throw
new
Nette\UnexpectedValueException("Unable to create service '$name', value returned by factory '$factory' is not '{$this->types[$name]}' type.");}unset($this->factories[$name]);return$this->registry[$name]=$service;}function
getServiceByType($type){foreach($this->registry
as$name=>$service){if(isset($this->types[$name])?!strcasecmp($this->types[$name],$type):$service
...
isset($this->types[$name])?!strcasecmp($this->types[$name],$type):(isset($this->registry[$name])&&$this->registry[$name]instanceof$type);}function
setParams(array$params){$this->updating();$this->params=$params;return$this;}function
getParams(){return$this->params;}function
setParam($key,$value){$this->updating();$this->params[$key]=$value;return$this;}function
getParam($key,$default=NULL){if(array_key_exists($key,$this->params)){return$this->params[$key];}elseif(func_num_args()>1){return$default;}else{throw
new
Nette\InvalidStateException("Missing parameter '$key'.");}}function
expand($s){if(is_string($s)&&strpos($s,'%')!==FALSE){$that=$this;return@preg_replace_callback('#%([a-z0-9._-]*)%#i',function($m)use($that){list(,$param)=$m;$val=$param===''?'%':$that->getParam($param);if(!is_scalar($val)){throw
new
Nette\InvalidStateException("Parameter '$param' is not scalar.");}return$val;},$s);}return$s;}function&__get($name){if($name==='params'){return$this->params;}elseif(!isset($this->registry[$name])){$this->getService($name);}return$this->registry[$name];}function
...
Helpers{static
function
editorLink($file,$line){$dir=dirname(strtr($file,'/',DIRECTORY_SEPARATOR));$base=isset($_SERVER['SCRIPT_FILENAME'])?dirname(dirname(strtr($_SERVER['SCRIPT_FILENAME'],'/',DIRECTORY_SEPARATOR))):dirname($dir);if(substr($dir,0,strlen($base))===$base){$dir='...'.substr($dir,strlen($base));}if(Debugger::$editor){$el=Nette\Utils\Html::el('a')->href(strtr(Debugger::$editor,array('%file'=>rawurlencode($file),'%line'=>$line)));}else{$el=Nette\Utils\Html::el('span');}return$el->title("$file:$line")->setHtml(htmlSpecialChars(rtrim($dir,DIRECTORY_SEPARATOR)).DIRECTORY_SEPARATOR.'<b>'.htmlSpecialChars(basename($file)).'</b>');}static
function
...
finalize(&$s){if(count($this->blocks)===1){$s.=$this->parser->macro('/block');}if($this->namedBlocks||$this->extends){$s='<?php
if ($_l->extends) {
	ob_start();
...
macroCache($content){return'if (Nette\Latte\DefaultMacros::createCache($netteCacheStorage, '.var_export($this->uniq.':'.$this->cacheCounter++,TRUE).', $_l->g->caches'.$this->formatArray($content,', ').')) {';}function
...
macroContentType($content){if(strpos($content,'html')!==FALSE){$this->parser->escape='Nette\Templating\DefaultHelpers::escapeHtml|';$this->parser->context=Parser::CONTEXT_TEXT;}elseif(strpos($content,'xml')!==FALSE){$this->parser->escape='Nette\Templating\DefaultHelpers::escapeXml';$this->parser->context=Parser::CONTEXT_NONE;}elseif(strpos($content,'javascript')!==FALSE){$this->parser->escape='Nette\Templating\DefaultHelpers::escapeJs';$this->parser->context=Parser::CONTEXT_NONE;}elseif(strpos($content,'css')!==FALSE){$this->parser->escape='Nette\Templating\DefaultHelpers::escapeCss';$this->parser->context=Parser::CONTEXT_NONE;}elseif(strpos($content,'plain')!==FALSE){$this->parser->escape='';$this->parser->context=Parser::CONTEXT_NONE;}else{$this->parser->escape='$template->escape';$this->parser->context=Parser::CONTEXT_NONE;}if(strpos($content,'/')){return'$netteHttpResponse->setHeader("Content-Type", "'.$content.'")';}}function
...
macroVar($content,$modifiers,$extract=FALSE){$out='';$var=TRUE;foreach($this->parseMacroArgs($content)as$token){if($var&&($token['type']===self::T_SYMBOL||$token['type']===self::T_VARIABLE)){if($extract){$out.="'".trim($token['value'],"'$")."'";}else{$out.='$'.trim($token['value'],"'$");}}elseif(($token['value']==='='||$token['value']==='=>')&&$token['depth']===0){$out.=$extract?'=>':'=';$var=FALSE;}elseif($token['value']===','&&$token['depth']===0){$out.=$extract?',':';';$var=TRUE;}else{$out.=$token['value'];}}return$out;}function
...
escape(){$tmp=explode('|',$this->parser->escape);return$tmp[0];}function
formatModifiers($var,$modifiers){if(!$modifiers){return$var;}$inside=FALSE;foreach($this->parseMacroArgs(ltrim($modifiers,'|'))as$token){if($token['type']===self::T_WHITESPACE){$var=rtrim($var).' ';}elseif(!$inside){if($token['type']===self::T_SYMBOL){if(trim($token['value'],"'")==='escape'){$tmp=explode('|',$this->parser->escape);$var=$tmp[0]."($var".(isset($tmp[1])?', '.var_export($tmp[1],TRUE):'');}else{$var="\$template->".trim($token['value'],"'")."($var";}$inside=TRUE;}else{throw
new
...
NULL;}function
formatMacroArgs($input){$out='';foreach($this->parseMacroArgs($input)as$token){$out.=$token['value'];}return$out;}function
formatArray($input,$prefix=''){$tokens=$this->parseMacroArgs($input);if(!$tokens){return'';}$out='';$expand=NULL;$tokens[]=NULL;foreach($tokens
...
Nette\Caching\Cache($cacheStorage,'Nette.Templating.Cache');if($helper=$cache->start($key)){$helper->dependencies=array(Nette\Caching\Cache::TAGS=>isset($args['tags'])?$args['tags']:NULL,Nette\Caching\Cache::EXPIRATION=>isset($args['expire'])?$args['expire']:'+ 7 days');$parents[]=$helper;}return$helper;}}class
Engine
extends
...
MacroNode
extends
Nette\Object{public$name;public$isEmpty=FALSE;public$attrs=array();public$args;public$modifiers;public$closing=FALSE;public$offset;function
__construct($name,$args=NULL,$modifiers=NULL){$this->name=$name;$this->args=$args;$this->modifiers=$modifiers;}}}namespace Nette\Templating{use
Nette;class
FilterException
extends
...
contextText(){$matches=$this->match('~
		...
		~xsi');if(!$matches||!empty($matches['macro'])||!empty($matches['comment'])){}elseif(!empty($matches['htmlcomment'])){$this->context=self::CONTEXT_COMMENT;$this->escape='Nette\Templating\DefaultHelpers::escapeHtmlComment';}elseif(empty($matches['closing'])){if(Strings::startsWith($matches['tag'],self::HTML_PREFIX)){$node=new
...
contextTag(){$matches=$this->match('~
		...
		~xsi');if(!$matches||!empty($matches['macro'])||!empty($matches['comment'])){}elseif(!empty($matches['end'])){$node=end($this->htmlNodes);$isEmpty=!$node->closing&&(strpos($matches['end'],'/')!==FALSE||$node->isEmpty);if($isEmpty){$matches[0]=(Nette\Utils\Html::$xhtml?' />':'>').(isset($matches['tagnewline'])?$matches['tagnewline']:'');}if($node
instanceof
MacroNode||!empty($node->attrs)){if($node
instanceof
MacroNode){$code=$this->tagMacro(substr($node->name,strlen(self::HTML_PREFIX)),$node->attrs,$node->closing);if($isEmpty){$code.=$this->tagMacro(substr($node->name,strlen(self::HTML_PREFIX)),$node->attrs,TRUE);}}else{$code=substr($this->output,$node->offset).$matches[0].(isset($matches['tagnewline'])?"\n":'');$code=$this->attrsMacro($code,$node->attrs,$node->closing);if($isEmpty){$code=$this->attrsMacro($code,$node->attrs,TRUE);}}$this->output=substr_replace($this->output,$code,$node->offset);$matches[0]='';}if($isEmpty){$node->closing=TRUE;}if(!$node->closing&&(strcasecmp($node->name,'script')===0||strcasecmp($node->name,'style')===0)){$this->context=self::CONTEXT_CDATA;$this->escape='Nette\Templating\DefaultHelpers::escape'.(strcasecmp($node->name,'style')?'Js':'Css');}else{$this->context=self::CONTEXT_TEXT;$this->escape='Nette\Templating\DefaultHelpers::escapeHtml|';if($node->closing){array_pop($this->htmlNodes);}}}else{$name=$matches['attr'];$value=isset($matches['value'])?$matches['value']:'';if($isSpecial=Strings::startsWith($name,self::HTML_PREFIX)){$name=substr($name,strlen(self::HTML_PREFIX));}$node=end($this->htmlNodes);if($isSpecial||$node
instanceof
MacroNode){if($value==='"'||$value==="'"){if($matches=$this->match('~(.*?)'.$value.'~xsi')){$value=$matches[1];}}$node->attrs[$name]=$value;$matches[0]='';}elseif($value==='"'||$value==="'"){$this->context=self::CONTEXT_ATTRIBUTE;$this->quote=$value;$this->escape=strncasecmp($name,'on',2)?('Nette\Templating\DefaultHelpers::escapeHtml'.(strcasecmp($name,'style')?"|$value":'Css')):"Nette\\Templating\\DefaultHelpers::escapeHtmlJs|$value";}}return$matches;}private
function
...
macro($name,$args='',$modifiers=''){if(!isset($this->macros[$name])){throw
new
ParseException("Unknown macro {{$name}}",0,$this->line);}$closing=$name[0]==='/';if($closing){$node=array_pop($this->macroNodes);if(!$node||"/$node->name"!==$name||($args&&!Strings::startsWith("$node->args ","$args "))||$modifiers){$name.=$args?' ':'';throw
new
ParseException("Unexpected macro {{$name}{$args}{$modifiers}}".($node?", expecting {/$node->name}".($args&&$node->args?" or eventually {/$node->name $node->args}":''):''),0,$this->line);}$node->args=$node->modifiers='';}else{$node=new
MacroNode($name,$args,$modifiers);if(isset($this->macros["/$name"])){$node->isEmpty=TRUE;$this->macroNodes[]=$node;}}$handler=$this->handler;return
Strings::replace($this->macros[$name],'#%(.*?)%#',function($m)use($handler,$node){if($m[1]){return
callback($m[1][0]===':'?array($handler,substr($m[1],1)):$m[1])->invoke($node->args,$node->modifiers);}else{return$handler->formatMacroArgs($node->args);}});}function
tagMacro($name,$attrs,$closing){$knownTags=array('include'=>'block','for'=>'each','block'=>'name','if'=>'cond','elseif'=>'cond');if(!isset($this->macros[$name])){throw
new
ParseException("Unknown tag-macro <$name>",0,$this->line);}return$this->macro($closing?"/$name":$name,isset($knownTags[$name],$attrs[$knownTags[$name]])?$attrs[$knownTags[$name]]:preg_replace("#'([^\\'$]+)'#",'$1',substr(var_export($attrs,TRUE),8,-1)),isset($attrs['modifiers'])?$attrs['modifiers']:'');}function
attrsMacro($code,$attrs,$closing){foreach($attrs
as$name=>$args){if(substr($name,0,5)==='attr-'){if(!$closing){$pos=strrpos($code,'>');if($code[$pos-1]==='/'){$pos--;}$code=substr_replace($code,str_replace('@@',substr($name,5),$this->macro("@attr",$args)),$pos,0);}unset($attrs[$name]);}}$left=$right=array();foreach($this->macros
...
new
ParseException("Unknown macro-attribute ".self::HTML_PREFIX.implode(' and '.self::HTML_PREFIX,array_keys($attrs)),0,$this->line);}$s='';foreach($left
as$item){$m=$this->macro($item[0],$item[1]);$s.=$m.(substr($m,-2)==='?>'?"\n":'');}$s.=$code;foreach($right
as$item){$m=$this->macro($item[0],$item[1]);$s.=$m.(substr($m,-2)==='?>'?"\n":'');}$s=rtrim($s,"\n");return$s;}function
...
array($match['name'],trim($match['args']),$match['modifiers']);}}}namespace Nette\Loaders{use
Nette;use
Nette\Utils\Strings;use
Nette\Caching\Cache;class
RobotLoader
...
Nette\FatalErrorException($error['message'],0,$error['type'],$error['file'],$error['line'],NULL),TRUE);}if(self::$bar&&!self::$productionMode&&!self::$ajaxDetected&&!self::$consoleMode&&!preg_match('#^Content-Type: (?!text/html)#im',implode("\n",headers_list()))){self::$bar->render();}}static
function
_exceptionHandler(\Exception$exception,$drawBar=FALSE){if(!headers_sent()){header('HTTP/1.1 500 Internal Server Error');}$htmlMode=!self::$ajaxDetected&&!preg_match('#^Content-Type: (?!text/html)#im',implode("\n",headers_list()));try{if(self::$productionMode){self::log($exception,self::ERROR);if(self::$consoleMode){echo"ERROR: the server encountered an internal error and was unable to complete your request.\n";}elseif($htmlMode){?>
<!DOCTYPE html>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<meta name=robots content=noindex><meta name=generator content="Nette Framework">
<style>body{color:#333;background:white;width:500px;margin:100px auto}h1{font:bold 47px/1.5 sans-serif;margin:.6em 0}p{font:21px/1.5 Georgia,serif;margin:1.5em 0}small{font-size:70%;color:gray}</style>
...
<?php }}else{if(self::$consoleMode){echo"$exception\n";}elseif($htmlMode){self::$blueScreen->render($exception);if($drawBar&&self::$bar){self::$bar->render();}}elseif(!self::fireLog($exception,self::ERROR)){self::log($exception);}}foreach(self::$onFatalError
